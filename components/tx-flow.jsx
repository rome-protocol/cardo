...
//
// Ported from designer's V3 delivery (src/tx-flow.jsx). Visual layer is
// byte-preserved; only module-system seams adapted (imports + ES exports).
// `Eyebrow` is imported from the sibling primitives module rather than
// read off window.
...
const useTxFlow = () => {
  ...
  // `setTxHash` exposed so real-tx callers (e.g. /swap direct-precompile path)
  // can patch in the actual on-chain hash without replaying `run()`'s sim.
  return { state, txHash, run, reset, setState, setTxHash };
};

// Tx modal — step stepper for the state machine.
// `errorMessage` (optional) — when in `failed` state, display the real
// revert reason instead of the hardcoded SlippageExceeded placeholder.
const TxModal = ({ flow, title, summary, onClose, onRetry, steps, errorMessage }) => {
  ...
  // `done` is true the moment we transition into the terminal "confirmed"
  // state. Once done, every step (including the last "confirmed" step
  // itself) shows as completed — without the override below the last
  // step would render with the active spinner because `i === idx` for
  // the terminal step.
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        {!failed && (
          <ol style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 2 }}>
            {stateSteps.map((s, i) => {
              // When the flow has reached the terminal state, every step
              // — including the terminal one — is "done". Otherwise use
              // the position-vs-current mapping.
              const state = done
                ? "done"
                : i < idx
                ? "done"
        ...
        {failed && (
          <div style={{ padding: 16, borderRadius: 10, background: "rgba(180,68,42,0.06)", border: "1px solid rgba(180,68,42,0.2)", marginBottom: 18 }}>
            <div className="mono" style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.1em", color: "#8a3320", marginBottom: 8 }}>Revert</div>
            <p style={{ margin: 0, fontSize: 13, color: "var(--fg1)", wordBreak: "break-word" }}>
  ...
  <div className="modal-scrim" onClick={onClose}>
    <div className="modal" onClick={(e) => e.stopPropagation()}>
      ...
      <p className="small" style={{ marginTop: 18, color: "var(--fg2)" }}>
        Cardo requests <span className="mono" style={{ fontSize: 12 }}>eth_requestAccounts</span> and, on first tx, a network switch to Rome chain (chainId {chainId}).
      </p>
    </div>
  </div>
  );
...
export { useTxFlow, TxModal, ConnectModal };
